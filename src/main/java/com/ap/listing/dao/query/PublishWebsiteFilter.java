...
@Component
public class PublishWebsiteFilter extends InitQuery<WebsitePublisher> {

    @Override
    public List<Predicate> initFilter(ListPayload listPayload, Root<WebsitePublisher> root) {
            ...
            listPayload
                    ...
                    .forEach(filter -> {
                        ...
                        if (Objects.nonNull(filter.getNestedPath())) {
                            Join<WebsitePublisher, WebsiteCategory> roleJoin = root.join(filter.getNestedPath());
    ...
    private void normalFilter(Root<WebsitePublisher> root, Filter filter, List<Predicate> selectionPredicates, List<Predicate> filterPredicates) {
        if (filter.isPartialSearch()) {
            filter
                    .getSelections()
                    .forEach(selection ->
