...
@Slf4j
public class PrepareTaskBuyLinkInsertion {

    private final ModelMapper modelMapper;
    private final AdvertPulseProperties advertPulseProperties;

    public TaskPublisher preparePublisherTask(BuyLinkInsertionRequest buyLinkInsertionRequest, WebsitePublisher websitePublisher) {
        TaskPublisher taskPublisher = new TaskPublisher();
        Date now = new Date();
        List<PricingPayload> pricingPayloads = preparePricingPayload(buyLinkInsertionRequest, websitePublisher);
        List<PricingPayload> finalPricingPayloads = prepareFinalPricingPayload(pricingPayloads);
        taskPublisher.setProductType(ProductType.LINK_INSERTION.name());
        ...
        //taskPublisher.setContent(""); //Todo: Need to confirm as there is no content field in Link Insertion
        taskPublisher.setSpecialRequirements(buyLinkInsertionRequest.getSpecialRequirements());
        taskPublisher.setUrlAnchorTexts(Collections.singletonList(buyLinkInsertionRequest.getUrlAnchorTexts()));
    ...
    private List<PricingPayload> preparePricingPayload(BuyLinkInsertionRequest buyLinkInsertionRequest, WebsitePublisher websitePublisher) {
        ...
        PricingPayload pricingPayload = PricingPayload
                ...
                .price(websitePublisher.getLinkInsertionPrice())
                .build();
    ...
    private List<PricingPayload> prepareFinalPricingPayload(List<PricingPayload> pricingPayloads) {
        List<PricingPayload> finalPricingPayLoad = new ArrayList<>(pricingPayloads); //TODO:
        double totalPrice = prepareTotalPrice(finalPricingPayLoad);
        double contentPlacement = advertPulseProperties.getPlatformFee().getLinkInsertion();
        ...
        PricingPayload payload = PricingPayload
                ...
                .price(platformFee)
                .build();
        finalPricingPayLoad.add(payload);
        log.info("Pricing Payload prepared with response {}", finalPricingPayLoad);
        return finalPricingPayLoad;
    }
